...
**Current Coverage**: 84.62% overall, 11/11 business logic files ≥75% ✅
...
- `unified-portal-oracle` → `Schemas` → `UMDUSER` → `Tables`
- You should see all application tables (users, catalogues, categories, etc.)
- Right-click on any table → **View Data** to see the contents
...
1. **First**: Check if user has specific catalogue-level permission → If yes, grant access (refinement)
2. **Second**: Check if user has menu-level permission → If yes, grant access to all catalogues in menu (default)
...
**Access:**
- Navigate to Admin Panel → "User-Role Management" tab
...
2. Assign menu permission: "Storage Menu Admin" → "Storage" menu (admin permission)
3. Assign role to user: User → "Storage Menu Admin"
...
2. Assign menu permission: "Backup Configuration Access" → "Backup" menu (read permission) - needed to show menu
3. Assign catalogue permission: "Backup Configuration Access" → "Backup Configuration" catalogue (read permission)
4. Assign role to user: User → "Backup Configuration Access"
...
**Solution**:
1. Assign role: User → "Storage Menu Admin"
2. Assign role: User → "Firewall Menu Admin"
...
- ✅ Same functionality as official Oracle image
- ✅ Automatically creates pluggable database `XEPDB1`
- ✅ Easy to use with simple environment variables
...
**Benefits:**
- ✅ More reliable than direct wget downloads (avoids 404 errors)
- ✅ No authentication issues (uses official Oracle repository)
- ✅ Automatically gets compatible version
- ✅ Smaller final image (only necessary libraries copied)
